...
from model.transforms_3d import ExpandChannel, Normalize, LabelEncode, ExpandChannelWithLabel
...
def evaluation(net, num_classes):
    eval_dataset = tumour(isTrain=False)
    eval_dataset = GeneratorDataset(source=eval_dataset, column_names=['img', 'label'],
                          num_parallel_workers=1, shuffle=False)
    trans = Compose([Normalize(),
                     # LabelEncode(),
    ...
    for batch in tqdm(eval_dataset.create_dict_iterator(num_epochs=1), total=eval_data_size):
        ...
        label = label.asnumpy()
        # for acc
        area_intersect, area_pred_label, area_label = calculate_iou(pred, label, num_classes=3)
    ...
    print("    background: {:.2f}, kidney: {:.2f}, tumour: {:.2f} ".format(dice[0]*100, dice[1]*100, dice[2]*100))
    return dice[0]*100, dice[1]*100, dice[2]*100