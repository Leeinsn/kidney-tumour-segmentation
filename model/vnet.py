...
class OutputTransition(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, elu):
        ...
        self.conv1 = nn.Conv3d(in_channels, out_channels, kernel_size=(5, 5, 5), pad_mode='pad', padding=2)
        self.bn = nn.BatchNorm3d(out_channels)
        self.conv2 = nn.Conv3d(out_channels, out_channels, kernel_size=(3, 3, 3), pad_mode='pad', padding=1)
    ...
    def construct(self, x):
        # 2*32*64*128*128
        out = self.relu(self.bn(self.conv1(x))) # 2*2*64*128*128
        out = self.conv2(out)
        # B, C, X, Y, Z = out.shape  # 2*2*64*128*128
        # out = out.view(B, C, -1)  # 2*2, 64*128*128
...
class VNet(nn.Cell):
    ...
    def __init__(self, dropout=True, elu=True):
        ...
        self.out_tr = OutputTransition(32, 3, elu) # num_classes = 3

    def construct(self, x):
        """vnet construct"""

        out16 = self.in_tr(x)   # 2*16*64*128*128
        out32 = self.down_tr32(out16) # 2*32*32*64*64
        out64 = self.down_tr64(out32)  # 2*64*16*32*32
        out128 = self.down_tr128(out64) # 2*128*8*16*16
        out256 = self.down_tr256(out128)  # 2*256*4*8*8
        out = self.up_tr256(out256, out128) # 2*256*8*16*16
        out = self.up_tr128(out, out64) # 2*128*16*32*32
        out = self.up_tr64(out, out32) # 2*64*32*64*64
        out = self.up_tr32(out, out16) # 2*32*64*128*128
        out = self.out_tr(out)
        return out
